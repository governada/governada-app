...
export async function GET(_request: Request, { params }: { params: Promise<{ epoch: string }> }) {
  try {
    ...
    if (isNaN(epochNo)) {
      return new ImageResponse(
        <OGBackground>
          ...
          >
            <div style={{ display: 'flex', fontSize: '36px', color: OG.textMuted }}>
              Report not found
            </div>
          </div>
        </OGBackground>,
                ...
                {ghiDelta != null && ghiDelta !== 0 && (
                  <div
                    style={{
                      display: 'flex',
                      fontSize: '20px',
                      color: ghiDelta > 0 ? OG.green : OG.red,
                    }}
                  >
                    {ghiDelta > 0 ? '+' : ''}
                    {ghiDelta}
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '16px', flex: 1 }}>
              {[
                ...
                { label: 'ADA Governed', value: data.stats.totalAdaGoverned, accent: OG.green },
                {
                  ...
                  value: `${data.stats.avgParticipation}%`,
                  accent: OG.amber,
                },
              ].map((s) => (
                <div key={s.label} style={{ display: 'flex', alignItems: 'center', gap: '16px' }}>
                  ...
                  <div
                    style={{ display: 'flex', fontSize: '28px', fontWeight: 700, color: s.accent }}
                  >
                    {s.value}
                  </div>
                </div>
