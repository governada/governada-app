  ...
  async ({ step }) =>
    withCronMonitor(
      ...
      async () => {
        ...
        const constitutionalResult = await step.run(
        ...
        if (classifyResult.skipped) {
          ...
          await step.run('heartbeat-alignment', () => pingHeartbeat('HEARTBEAT_URL_ALIGNMENT'));
            ...
            await sb
              ...
              .update({
                ...
                error_message: capMsg(
                  'skipped: ' +
                    ('reason' in computeResult ? String(computeResult.reason) : 'unknown'),
                ),
                metrics: {
                  skipped: true,
