...
export const GET = withRouteHandler(async (request, { requestId }) => {
  ...
  const currentIndex = allDreps.findIndex((d) => d.id === drepId);
  if (currentIndex === -1) {
  ...
  const nearbyBelow = allDreps.slice(currentIndex + 1, currentIndex + 3).map((d, i: number) => ({
    drepId: d.id,
    name: d.metadata?.givenName || d.id.slice(0, 16),
    score: d.score,
  ...
  const top10Avg = {
    participation: Math.round(
      top10.reduce((s: number, d) => s + (d.effective_participation || 0), 0) / top10.length,
    ),
    rationale: Math.round(
      top10.reduce((s: number, d) => s + (d.rationale_rate || 0), 0) / top10.length,
    ),
    reliability: Math.round(
      top10.reduce((s: number, d) => s + (d.reliability_score || 0), 0) / top10.length,
    ),
    profile: Math.round(
      top10.reduce((s: number, d) => s + (d.profile_completeness || 0), 0) / top10.length,
    ),
  };
