...
| J04 | Anonymous citizen  | Enter Quick Match on the homepage and progress into live results | "Governada can guide me to representation"                 | L0         | `e2e/critical-public.spec.ts`, `e2e/quick-match.spec.ts`, `__tests__/app/match-page.test.tsx`, `__tests__/app/match-experience-client.test.tsx` | Confidence is still quiz-weighted until proposal votes deepen the profile | Keep the homepage workspace in the blocking gate and use `/match` as an alias only while confidence deepens via `/match/vote` |
...
2. The public-path gate now blocks PRs, but proposal detail assertions are still lighter than launch-end-state proofs and Match confidence is still quiz-weighted until real vote history deepens it.
...
3. Keep the homepage-owned `/?mode=match` workspace in the blocker, preserve `/match` only as an alias, and deepen result-state plus accessibility coverage over time.
