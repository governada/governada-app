...
- `/developers` → keep as standalone route outside `(civica)` group, similar to `/embed/*` and `/admin/*`. Developer docs are a different audience (API consumers) and don't need the civic shell. Link to it from footer and ⌘K only.
...
- `civica_segment_detected` — segment type, wallet connected (yes/no)
- `civica_first_civic_action` — action type (delegate, match, share), time from landing (measures < 2 min target)
- `civica_quick_match_complete` — duration, result count, delegated (yes/no)
- `civica_tier_celebration_shown` — tier, entity type, shared (yes/no)
- `civica_share_triggered` — share type (X, link, download), content type (tier, achievement, profile, wrapped)
- `civica_action_feed_cta_clicked` — card type, action taken
- `civica_destination_viewed` — destination (Home/Discover/Pulse/My Gov), segment
- `civica_drift_alert_shown` — action taken (re-delegate, dismiss, view alternatives)
- `civica_education_chip_expanded` — term name (measures contextual education engagement)
