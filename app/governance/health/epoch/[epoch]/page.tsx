...
export default async function EpochReportPage({ params }: Props) {
  const { epoch } = await params;

  if (epoch === 'latest') {
    const report = await getReport('latest');
    if (report) redirect(`/governance/health/epoch/${report.epoch_no}`);
    notFound();
  }
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 py-6 space-y-8">
      ...
      <div className="border-t pt-8 space-y-4">
        <ShareActions
          url={`https://governada.io/governance/health/epoch/${data.epoch}`}
