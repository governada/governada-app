  ...
  async ({ step }) =>
    withCronMonitor(
      ...
      async () => {
        const result = await step.run('compute-drep-scores', async () => {
          ...
          try {
            ...
            }> = [];
            {
              ...
              logger.info('[scoring] Loaded drep_votes', {
            ...
            let semanticDiversity: Map<string, number> | undefined;
            {
              ...
              if (embCount && embCount > 0) {
                ...
                logger.info(
                  '[scoring] No rationale embeddings found, using meta_hash diversity only',
                );
              }
            }
            ...
            const rawIdentity = computeGovernanceIdentity(
              profiles,
              delegationSnapshots,
              nowSeconds,
            );
            ...
            for (const [drepId, votes] of drepVotes) {
              ...
              const typeCoverage =
                allProposalTypes.size > 0 ? types.size / allProposalTypes.size : 0;
        ...
        await step.run('heartbeat-scoring', () => pingHeartbeat('HEARTBEAT_URL_SCORING'));
        return result;
      },
      { maxRuntime: 45 },
    ),
