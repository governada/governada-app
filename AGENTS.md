...
- Persist approvals for safe recurring prefixes such as `npm run worktree:new`, `npm run worktree:sync`, `npm run session:doctor`, `npm run session:guard`, `npm run gh:auth-status`, `npm run github:runtime-doctor`, `npm run github:read-doctor`, `npm run github:write-doctor`, `npm run auth:repair`, `npm run ci:watch`, `npm run ci:failed`, `npm run pre-merge-check`, `npm run deploy:verify`, `npm run inngest:register`, `git add`, `git commit -m`, `git push`, `git fetch origin main`, `git worktree add`, and `gh api repos/governada/app/pulls`.
...
- The Phase 0B `github.write.pr` lane is diagnosed by `npm run github:write-doctor`. `npm run github:pr-write` is the bounded PR metadata wrapper for this lane. It dry-runs by default and requests only `pull_requests=write` plus GitHub's implicit `metadata=read`. Live mode requires `--execute --confirm github.write.pr` plus an approved human-provided service-account token runtime, and is limited to draft PR creation, title/body updates on draft PRs after a read-before-write draft check, or marking draft PRs ready for review. It does not authorize comments, branch pushes, merge, deploy, production mutation, admin, billing, or secret changes.
- The Phase 0B service-account runtime posture is diagnosed by `npm run github:runtime-doctor`. `OP_SERVICE_ACCOUNT_TOKEN` must remain out of files and may be supplied only through an approved runtime; when it is present, token-bearing GitHub App doctors and wrappers require non-secret expiry/rotation metadata (`GOVERNADA_OP_SERVICE_ACCOUNT_EXPIRES_AT` and `GOVERNADA_OP_SERVICE_ACCOUNT_ROTATE_AFTER`) so agents can warn before expiry and block unsafe near-expiry use.
...
- Before generic troubleshooting, run `npm run session:doctor`, then `npm run gh:auth-status`, then inspect `.mcp.json` and the referenced wrapper commands. For the autonomous GitHub App lane, run `npm run github:runtime-doctor` first, then `npm run github:read-doctor` or `npm run github:write-doctor` for the operation class. These doctors may inspect `.env.local.refs` for GitHub App IDs, rotation metadata, and the private-key reference, but `OP_SERVICE_ACCOUNT_TOKEN` must come from an approved secure runtime environment or mount.
