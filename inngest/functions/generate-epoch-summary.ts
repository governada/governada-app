...
export const generateEpochSummary = inngest.createFunction(
  ...
  async ({ step }) =>
    withCronMonitor('generate-epoch-summary', '0 22 * * *', async () => {
...
Output ONLY the narrative paragraph, nothing else.`,
            ...
            await supabase.from('proposal_vote_snapshots').upsert(
              {
                ...
                drep_yes_power: drepYes.reduce(
                  (s, v) => s + Number(v.voting_power_lovelace || 0),
                  0,
                ),
      ...
      await step.run('heartbeat-epoch-summary', () => pingHeartbeat('HEARTBEAT_URL_EPOCH_SUMMARY'));
      return {
        epoch,
        usersProcessed,
        ...proposalStats,
