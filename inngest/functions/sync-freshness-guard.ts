...
export const syncFreshnessGuard = inngest.createFunction(
  {
    ...
    triggers: { cron: '14,44 * * * *' }, // Offset from :00/:30 to run after primary syncs
  },
  async ({ step }) =>
    withCronMonitor('sync-freshness-guard', '14,44 * * * *', async () => {
        ...
        for (const ghost of ghosts) {
          await supabase
            ...
            .update({
              ...
              success: false,
              error_message:
                'Historical ghost: process never completed (cleaned by freshness guard)',
              duration_ms: 0,
            })
            .eq('id', ghost.id);
        }
