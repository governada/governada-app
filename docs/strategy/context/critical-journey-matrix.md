...
| J04 | Anonymous citizen  | Enter Quick Match from the public shell and begin the quiz flow | "Governada can guide me to representation"                 | L0         | `e2e/critical-public.spec.ts`, `e2e/quick-match.spec.ts`, `__tests__/components/QuickMatch.test.tsx`                   | Result-state and full interaction coverage are lighter  | Keep entry-state checks blocking; deepen quiz interaction and result-state assertions over time |
...
2. The public-path gate now blocks PRs, but proposal detail and result-state assertions are still lighter than launch-end-state proofs.
...
3. Add one stronger assertion to proposal detail and Quick Match result-state coverage, then graduate Match accessibility into the blocking gate.
