...
- Run `npm run session:refresh` before commands that produce evidence from local checkout state after a long session or after Tim says PRs were merged elsewhere. This fast-forwards the shared checkout when it is clean, or prints the exact WIP/untracked files that need attention before local status evidence can be trusted. Use this before local-state evidence commands such as `npm run inngest:status`, migration-list comparisons, or anything that reads `app/api/inngest/route.ts`, `.mcp.json`, or `supabase/migrations/`.
- Use `bin/betterstack.sh` and `bin/sentry.sh` for agent observability access. They resolve `BETTERSTACK_API_TOKEN_OP_REF` and `SENTRY_AUTH_TOKEN_OP_REF` through the same `OP_AGENT_SERVICE_ACCOUNT_TOKEN` lane as `bin/supabase-mcp.sh`, keep tokens out of logs, and pin Better Stack/Sentry calls to the Governada operational scopes.
