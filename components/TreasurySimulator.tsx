              ...
              <div className="space-y-1">
                ...
                {data.counterfactual.largestWithdrawals.map((w, i) => (
                  ...
                  >
                    ...
                    <span className="font-mono tabular-nums ml-4">
...
function ProjectionChart({ scenarios }: { scenarios: SimulationData['scenarios'] }) {
  ...
  const xScale = useMemo(
    () => scaleLinear().domain(epochExtent).range([0, innerWidth]),
    [epochExtent, innerWidth],
  );
  const yScale = useMemo(
    () =>
      scaleLinear()
        .domain([0, balanceMax * 1.05])
        .range([innerHeight, 0]),
    [balanceMax, innerHeight],
  );
                ...
                {paths.map((p) => (
                  <g key={p.key}>
                    <path
                      ...
                      fill="none"
                      stroke={p.color}
                      strokeWidth={p.isMain ? 2.5 : 1.5}
                      filter={`url(#sim-glow-${p.key})`}
                      opacity={0.3}
                    />
                    <path
                      d={p.d}
                      ...
                      stroke={p.color}
                      strokeWidth={p.isMain ? 2.5 : 1.5}
                      strokeDasharray={p.isDashed ? '5 5' : 'none'}
                      strokeLinecap="round"
                    />
                  </g>
                ))}
          ...
          {hoveredEpoch !== null && width > 0 && (
            ...
            >
              <div className="rounded-lg border bg-card p-2.5 shadow-xl text-xs backdrop-blur-sm dark:border-border/60 dark:bg-card/95">
                ...
                {scenarios.map((s) => {
                  const closest = s.balanceCurve.reduce(
                    (best, p) =>
                    ...
                    <div key={s.key} className="flex items-center justify-between gap-3">
                      <span className="flex items-center gap-1.5">
                        <span
                          ...
                          style={{ backgroundColor: SCENARIO_CHART_COLORS[s.key] || '#888' }}
                        />
                        {s.name}
                      </span>
                      <span className="font-mono tabular-nums">
                        {formatAda(closest.balanceAda)}
                      </span>
                    </div>
                  );
                })}
          ...
          {paths.map((p) => (
            <div key={p.key} className="flex items-center gap-1.5 text-xs text-muted-foreground">
              <span
                ...
                style={{ backgroundColor: p.color, borderStyle: p.isDashed ? 'dashed' : 'solid' }}
              />
              {p.name}
            </div>
          ))}
