...
| Release and incident execution      | Shipping quickly only works if bad changes are detected, contained, and learned from                | How risky is a change? What verification is required? What is the rollback path?                                         | `AGENTS.md`, `.agents/skills/ship/SKILL.md`, `scripts/github-merge.mjs`, `scripts/rollback.mjs`            |
