              ...
              {lb.hallOfFame.map((d: any) => (
                ...
                >
                  <div className="p-4 rounded-lg border border-amber-500/20 bg-amber-500/5 hover:bg-amber-500/10 transition-colors text-center">
                    ...
                    <p className="text-2xl font-bold tabular-nums text-green-600 dark:text-green-400 mt-1">
                      {d.score}
                    </p>
                    <p className="text-[10px] text-muted-foreground mt-0.5">
                      {d.days}+ days at Strong
                    </p>
                  </div>
                </Link>
              ))}
            ...
            <CardContent className="p-4 flex items-center justify-between">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-muted-foreground">
                    Weekly intelligence report with AI analysis
                  </p>
                </div>
              </div>
      ...
      {pulse && (
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          <StatCard
            ...
            label="ADA Governed"
            value={pulse.totalAdaGoverned}
          />
          <StatCard
            icon={<ScrollText className="h-5 w-5 text-amber-500" />}
            label="Active Proposals"
            value={String(pulse.activeProposals)}
          />
          <StatCard
            icon={<Users className="h-5 w-5 text-blue-500" />}
            label="Active DReps"
            value={String(pulse.activeDReps)}
          />
          <StatCard
            icon={<Vote className="h-5 w-5 text-indigo-500" />}
            label="Votes This Week"
            value={String(pulse.votesThisWeek)}
          />
        </div>
      )}
              ...
              {pulse.communityGap.map((g: any) => {
                ...
                return (
                  <Link
                    ...
                    href={`/proposals/${g.txHash}/${g.index}`}
                    className="block"
                  >
